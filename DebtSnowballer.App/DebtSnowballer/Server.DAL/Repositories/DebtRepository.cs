...
namespace Server.DAL.Repositories
	...
	public class DebtRepository : GenericRepository<Debt>, IDebtRepository
		...
		public new Debt Get(int id, string auth0UserId)
			{
		...
		public new IEnumerable<Debt> GetAll(string auth0UserId)
			{
			try
				{
				...
				{
				_logger.LogError(ex, "Error occurred while getting all Debt entities for Auth0UserId {Auth0UserId}", auth0UserId);
		...
		public new bool Remove(int id, string auth0UserId)
			...
			try
				{
				var entityToRemove = _DbContext.Set<Debt>().SingleOrDefault(e => e.Id == id && e.Auth0UserId == auth0UserId);
				_DbContext.Set<Debt>().Remove(entityToRemove);
				return true;
				}
			catch (Exception ex)
				{
				_logger.LogError(ex, "Error occurred while removing Debt entity for Auth0UserId {Auth0UserId}", auth0UserId);
				return false;
				}
			}
		...
		public new bool Update(Debt entity, string auth0UserId)
			{
			try
				{
				var existingEntity = _DbContext.Set<Debt>().SingleOrDefault(e => e.Id == entity.Id && e.Auth0UserId == auth0UserId);
				if (existingEntity == null)
					{
					_logger.LogError("Entity not found in {RepositoryName} with ID: {Id}", nameof(DebtRepository), entity.Id);
					return false;
					}
				...
				{
				_logger.LogError(ex, "Error occurred while updating Debt entity for Auth0UserId {Auth0UserId}", auth0UserId);
				return false;
				}
			}
		}
