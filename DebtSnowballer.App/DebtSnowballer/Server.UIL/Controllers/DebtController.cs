...
namespace Server.UIL.Controllers
	...
	public class DebtController : ControllerBase
		...
		public IActionResult Post([FromBody] DebtDto requestDto)
			{
			_logger.LogInformation("Received {HttpMethod} request in {ControllerName} with DTO: {RequestDto}", "POST", nameof(DebtController), requestDto);

			if (!ModelState.IsValid)
				{
				_logger.LogError("Invalid {HttpMethod} attempt in {ControllerName} with DTO: {RequestDto}", "POST", nameof(DebtController), requestDto);
			...
			_logger.LogInformation("Successfully created entity in {ControllerName} with ID: {Id}", nameof(DebtController), mappedResult.Id);
		...
		public IActionResult Put(int id, [FromBody] DebtDto requestDto)
			{
			_logger.LogInformation("Received {HttpMethod} request in {ControllerName} with DTO: {RequestDto}", "PUT", nameof(DebtController), requestDto);

			if (!ModelState.IsValid)
				{
				_logger.LogError("Invalid {HttpMethod} attempt in {ControllerName} with DTO: {RequestDto}", "PUT", nameof(DebtController), requestDto);
				return BadRequest(ModelState);
				}
			...
			_logger.LogInformation("Successfully updated entity in {ControllerName} with ID: {Id}", nameof(DebtController), existingEntity.Id);
		...
		public IActionResult Delete(int id)
			{
			_logger.LogInformation("Received {HttpMethod} request in {ControllerName} with ID: {Id}", "DELETE", nameof(DebtController), id);

			var dbResult = Repository.Get(id);
			Repository.Remove(dbResult);
			UnitOfWork.SaveChanges();

			_logger.LogInformation("Successfully deleted entity in {ControllerName} with ID: {Id}", nameof(DebtController), id);
		...
		public IActionResult Get()
			...
			_logger.LogInformation("Exiting {HttpMethod} request in {ControllerName} with mappedResult: {MappedResult}", "GET", nameof(DebtController), mappedResult);
		...
		public IActionResult Get(int id)
			{
			_logger.LogInformation("Received {HttpMethod} request in {ControllerName} with ID: {Id}", "GET", nameof(DebtController), id);
